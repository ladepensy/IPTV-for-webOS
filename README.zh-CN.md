...
本项目只是播放器，不是频道或内容提供方。项目不会内置播放列表、频道、节目单或媒体流。请只添加你拥有或依法获授权访问的 M3U、XMLTV、图片和媒体来源。请勿使用本应用绕过 DRM、访问未经授权的广播，或重新分发受版权保护的内容。你需要自行遵守适用于每个播放源的法律、许可证、服务条款和网络策略。

## 隐私声明

应用会向你配置的播放列表、节目单、图片和媒体地址发送请求。为记住播放源，包含 URL 以及你配置的请求元数据在内的播放源设置可能会保存在电视本地应用存储中。本项目不主动包含广告或分析服务。除非你已了解电视和配置的服务会接收这些信息，否则请不要输入凭据或私人地址；也不要分享包含这些信息的截图、日志或导出的应用数据。
...
- Favorites 固定在分组列表顶部；频道行右侧星标按钮支持遥控器与鼠标收藏，并按播放源独立保存
...
| 左 | 频道库关闭时打开频道库；打开后返回上一列；到达 M3U 列后保持不动 |
...
界面隐藏时，导航上/下会直接换台并显示频道信息，左键或右键都会打开频道库，其他普通按键和指针操作只显示信息界面；Back 会弹出系统退出确认。连续 5 秒没有操作后，顶部状态、频道库、正在播放信息和操作提示都会隐藏，底层视频不会暂停或停止。播放失败或结束时，频道库未打开且界面可见时按 OK 会优先直接重播当前频道；频道库打开时仍可正常进入分组或确认新频道。
