...
Favorites 是固定在 All 后面的虚拟分组，内容来自当前播放源独立保存的收藏频道。频道列的
`focusedChannelControl` 表示行内焦点：`0` 为频道主体，`1` 为右侧收藏按钮。

频道库子状态机的 OK/确认结果分为四种：
...
| `FAVORITE_TOGGLED` | 收藏或取消收藏当前频道并保持频道库打开 | 否 |
...
| --- | --- | --- | --- |
| `←` | 打开频道库 | 打开频道库 | 收藏按钮返回频道主体；频道主体返回上一列；M3U 列保持不动 |
| `→` | 打开频道库 | 打开频道库 | 进入下一列；频道主体进入右侧收藏按钮 |
| `↑ / ↓` | 直接换台并显示信息 | 直接换台 | 在当前列移动焦点，不立即换台 |
| `OK` | 显示信息 | 打开频道库 | M3U/分组列进入下一列；频道主体播放并关闭频道库；收藏按钮切换收藏状态 |
...
频道行由两个并列的 Spotlight 控件组成。频道主体继续负责播放；右侧星标按钮负责收藏。
Favorites 中取消收藏后，焦点顺延到下一条收藏频道；删除最后一条后显示空状态，OK 不触发隐藏频道的播放。
收藏键按 `home-iptv:favorites` 保存，并按播放源 ID 隔离。
